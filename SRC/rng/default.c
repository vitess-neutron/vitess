...
gsl_rng_env_setup (void)
{
  ...
  if (p)
    {
      ...
      if (gsl_rng_default == 0)
        {
          ...
          fprintf (LogFilePtr, "GSL_RNG_TYPE=%s not recognized\n", p);
          fprintf (LogFilePtr, "Valid generator types are:\n");

          for (t = t0; *t != 0; t++)
            {
              fprintf (LogFilePtr, " %18s", (*t)->name);
      ...
      fprintf (LogFilePtr, "GSL_RNG_TYPE=%s\n", gsl_rng_default->name);
    }
  else
    {
  ...
  if (p)
    {
      seed = strtoul (p, 0, 0);
      fprintf (LogFilePtr, "GSL_RNG_SEED=%lu\n", seed);
    };
