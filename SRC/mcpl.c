...
void mcpl_write_header(mcpl_outfile_t of)
{
  ...
  if (!mcpl_platform_is_little_endian())
    start[7] = 'B';
  nb = fwrite(start, 1, sizeof(start), f->file);
  if (nb!=sizeof(start))
    mcpl_error(errmsg);
...
void mcpl_transfer_metadata(mcpl_file_t source, mcpl_outfile_t target)
{
  ...
  updg = mcpl_hdr_universal_pdgcode(source);
  if (updg)
...
int mcpl_tool(int argc,char** argv) 
{
  ...
  if (opt_extract) {
    ...
    fi = mcpl_open_file(filenames[0]);
    fo = mcpl_create_outfile(filenames[1]);
    ...
    left = opt_num_limit>0 ? (uint64_t)opt_num_limit : (uint64_t)-1;
    uint64_t added = 0;
    const mcpl_particle_t* particle;
