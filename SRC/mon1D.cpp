...
double Mon1D::DetermineParameter(VtMonPar id, Neutron* pNeutr)
{
  ...
  switch (id)
  {
    ...
    case K_Y:
      paramValue = GetKComponent(pNeutr->Vector, pNeutr->Wavelength, Y_AXIS);  // ky: y component of the wave vector
      break;
    case K_Z:
      paramValue = GetKComponent(pNeutr->Vector, pNeutr->Wavelength, Z_AXIS);  // kz: z component of the wave vector
      break;
