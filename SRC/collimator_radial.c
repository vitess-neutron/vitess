...
short  AdvanceToCylinderSurface(double* pTime, Neutron* pNeutron, double Distance, double Height, double Speed); // calculates the flight time to the entrance or exit wall and the point of intersection
...
double AngCentre =0.0,         /* -a  [deg]  direction to the center of the collimator     */
       AngWidth  =0.0,         /* -w  [deg]  angular width of the collimator               */
       OscWidth  =0.0,         /* -o  [deg]  angular width of the oscillation              */
       EntrHeight=0.0,         /* -h  [cm]   entrance height of the collimator             */
       ExitHeight=0.0,         /* -H  [cm]   exit height of the collimator                 */
       Distance  =0.0,         /* -d  [cm]   distance origin - beginning of the collimator */
       Length    =0.0,         /* -l  [cm]   length of the collimator channels             */
       BladeWidth=0.0;         /* -s  [cm]   thickness of a blade (separating 2 channels)  */
       // frequency =0.0,      /*     [Hz]   oscillation frequency of the collimator       */
       // phase     =0.0;      /*     [deg]  phase of the oscillation at t=0               */

// Variables determined from input parameters or trajectory data
double AngSep    =0.0,         /*     [deg]  angular separation of the collimator channels in deg      */
       AngDead   =0.0,         /*     [deg]  angle covered by one blade at the entrance (seen from the sample centre) */
       AngMin    =0.0,         /*     [deg]  minimal angle to collimator entrance                      */
       AngMax    =0.0,         /*     [deg]  minimal angle to collimator entrance                      */
       AngCntrAct=0.0,         /*     [deg]  actual dir. to the coll. centre in deg (due to osc.width) */
       AngMinAct =0.0;         /*     [deg]  actual minimal angle to collimator entrance               */
VtOscill _eOscColl=VT_OSC_OFF; /*            option: oscillating collimator  0: no   1: yes, but only random phase   */
...
{
  long     i=0;
  int      iChanIn=0,   iChanOut=0;   /*         channels of entrance and exit of the neutron                    */
  short    bHit1=FALSE, bHit2=FALSE;  /*         flag: cylinder is hit within collimator height                  */
  double   VelocityReal=0.0,          /* [cm/ms] velocity of the neutron                                         */
           ToF1=0.0,    ToF2=0.0,     /* [ms]    times of flight to the collimator entrance and through it resp. */
           NewAngH=0.0, NewPosZ=0.0;  /*         hor. [deg] and vert. position [cm] of the neutron               */
  Neutron  OutNeutron,  EnterNeutron;

  InitNeutron(&OutNeutron);
  InitNeutron(&EnterNeutron);
...
short AdvanceToCylinderSurface(double* pTime, Neutron* pNeutron, double Dist, double Height, double Speed)
{
  ...
  *pTime = 0.0;
  ...
  // the cylinder is hit, but it might be on a disc, i.e. before the cylinder wall is reached
  if (bHitC)
  {
    for(i=0; i<3; i++)
