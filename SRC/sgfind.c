...
static int CBR_RMx(int *RRMx, const int *CBMxR, const int *RMx, const int *InvCBMxR)
{
  int i, BufMx[9];
...
static void RotateCBMxR(const int *RMx, const int *InvRMx, int *CBMxR, int *InvCBMxR)
{
  int i, BufMx[9];
...
static const T_RTMx *GetSMxWithSameRot(const int *WtdRotMx, const T_SgInfo *SgInfo, T_RTMx *BufMx)
{
  int iList, i;
  const T_RTMx *lsmx;
...
static int FindInvertableMx(const int *Mx, int *InvMx, int *nActive, int *irActive, int *icActive)
{
  int Init, deterMx, i;
...
static int FixAxes(const T_SgInfo *SgInfo, const T_SgInfo *GenSgI, const int *iGen, T_RTMx *CBMx, T_RTMx *InvCBMx,
                   int *FreeMx, int TryAgain)
{
  ...
  i = StartFixAxes(SgInfo, GenSgI, iGen, CBMx, InvCBMx, &SMxG, &SMxS_G, SingleFreeMx, &NextTryAgain);
  if (i != 1)
    return i;
  ...
  for (;;) {
    deterR_I_FMx = FindInvertableMx(R_I_FMx, InvR_I_FMx, &nActive, irActive, icActive);
    if (deterR_I_FMx == 0)
      break;
    ...
    for (iTrV = 0; iTrV < nTrV; iTrV++, TrV += 3) {
      for (i = 0; i < 3; i++) {
        S_G[i] = (CTBF / STBF) * ((SMxS_G.s.T[i] - SMxG.s.T[i] - TrV[i]) % STBF);
        RedSh[i] = 0;
      }
      ...
      i = FixAxes(SgInfo, GenSgI, &iGen[1], CBMx, InvCBMx, NextFreeMx, NextTryAgain);
      if (i != 0)
        return i;
    }
                        ...
                        const int *IniInvCBMxR, T_RTMx *CBMx, T_RTMx *InvCBMx)
{
  ...
  if (SgInfo->XtalSystem == XS_Monoclinic) {
    ...
    i = AlignUniqueAxis(SgInfo, GenSgI, BufCBMx.s.R, BufInvCBMx.s.R, &RMxAA);
    if (i != 1)
      return i;
...
const T_TabSgName *FindReferenceSpaceGroup(T_SgInfo *SgInfo, T_RTMx *CBMx, T_RTMx *InvCBMx)
{
  ...
  for (tsgn = TabSgName; tsgn->HallSymbol; tsgn++) {
    ...
    for (iList = 1; iList < GenSgI.nList; iList++, lrmxi++) {
      ...
      if ((lrmxi->Order == -1 || lrmxi->Order == -3) && GenSgI.Centric == 0 && GenSgI.InversionOffOrigin == 0)
        goto ReturnError;
    }
    ...
    if (OL_GenSgI == OL_SgInfo) {
      ...
      stat = CompleteCBMx(SgInfo, NewLI, &GenSgI, IniCBMxR, IniInvCBMxR, CBMx, InvCBMx);
      if (stat < 0)
        return NULL;
