...
double          omega, y_ch[10][2000], x_ch[10][2000],
...
static double phase(const double x_ch_k_j, const double y_ch_k_j, 
		    const double WL, const VectorType Dir, const VectorType Pos)
{
...
static int outOfPhase(int i, int j, int gates, 
		      const double phase0, const double WL, const VectorType Dir, const VectorType Pos) {
...
static int inPhase(int gates, double phase0, 
		   const double WL, const VectorType Dir, const VectorType Pos) {
...
void processNeutron (int i, int thread_i) {
  ...
  if (zerotime==1)
    {
      neutron.Time =
	fabs(fmod(neutron.Time + Phase/omega + coef_pi*M_PI/omega/2., coef_pi*M_PI/omega))
...
void OwnInit(int argc, char *argv[])
{
  ...
  GatesFilePtr = fopen("gates.dat","w");
  ...
  if (Option==1)
	...
	for(j=1; j< 2*Nchannels+2; j++)
	  {
	    ...
	    for(k=0;k<GatesNumber; k++) y_ch[k][j] = y_ch[k][j-1] + add;
	...
	for(k=0;k<GatesNumber/2; k++)
	  {
	    x_ch[k][0] = x_ch[k][1] = x_ch[k][2*Nchannels] = x_ch[k][2*Nchannels+1] =
	      - depth/2. * (1. - pow(2.*k/GatesNumber, expon));
	    x_ch[GatesNumber - 1 - k][0] = x_ch[GatesNumber - 1 - k][1] =
	      x_ch[GatesNumber - 1 - k][2*Nchannels] = x_ch[GatesNumber - 1 - k][2*Nchannels+1] =
	      depth/2. * (1. - pow(2.*k/GatesNumber, expon));
	  }
	...
	x_ch[GatesNumber - 1][0] = x_ch[GatesNumber - 1][1] =
	  x_ch[GatesNumber - 1][2*Nchannels] = x_ch[GatesNumber - 1][2*Nchannels+1] =
	  main_depth/2.;
      ...
      if(CurvGeomOption==1)
	{
	  fprintf(LogFilePtr,
		  ...
		  V_FROM_LAMBDA(optimal_wl)/2./omega,
		  V_FROM_LAMBDA(optimal_wl)/2./omega*pow((1 + sq(omega*diameter/V_FROM_LAMBDA(optimal_wl))), 1.5));
	  ...
	  GeomFilePtr = fopen(GeomFileName,"w");
	  {
		...
		for(k=0;k<GatesNumber/2; k++) {
		  int ind = (int)(pow(2.*k/GatesNumber, expon)*250.);
		  x_ch[k][j] = xx[ind];
		  y_ch[k][j] = yy[ind];
		  x_ch[GatesNumber - 1 - k][j] = xx[499 - ind];
		  y_ch[GatesNumber - 1 - k][j] = yy[499 - ind];
		}
	      }
	    /* print out gate coordinates */
      ...
      if(CurvGeomOption==2)
	{
	  ...
	  GeomFilePtr = fopen(GeomFileName,"w");
	  {
	    ...
	    x_ch[GatesNumber - 1][0] =
	      ...
	      x_ch[GatesNumber - 1][2*Nchannels+1] =
	      main_depth/2.;
